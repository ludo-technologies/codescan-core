...
Found while verifying the load-time port, not a pyscn change: **jscan's module analyzer does not detect `import()` at all** — `processDynamicImport` never fires, so no dynamic edge is ever built, and both the load-time depth exclusion and the older cycle-detection exclusion (#460) are currently unreachable end to end. `internal/analyzer/dependency_graph_test.go` already skips a test over this ("Dynamic imports not detected by parser"). Worth its own fix.

Core issues raised by this run:

- **pyscn's dependency topology duplicates `core/graph.ChainFinder`.** Both sides now condense the module graph and read depth and longest chains off the same condensation; pyscn's local copy additionally supports context cancellation and globally ranked top-N chains, which `core/graph` lacks. pyscn should adopt `core/graph`, and `core/graph` should absorb those two capabilities first
- **`calculateParseErrorPenalty` is language-independent scoring policy.** It is anchored to `coredomain.GradeAThreshold`/`GradeDThreshold` and now exists verbatim in both tools; it belongs in `core/domain` next to `LinearPenalty` and `DuplicationPenalty`
