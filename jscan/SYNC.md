...
- **Sync baseline SHA**: `1ab776e3588459aef12f549c480ab845a993dc9f` (2026-08-12)
...
Core issues raised by this run:

- **pyscn's dependency topology duplicates `core/graph.ChainFinder`.** Both sides now condense the module graph and read depth and longest chains off the same condensation; pyscn's local copy additionally supports context cancellation and globally ranked top-N chains, which `core/graph` lacks. pyscn should adopt `core/graph`, and `core/graph` should absorb those two capabilities first ([polyscan#56](https://github.com/ludo-technologies/polyscan/issues/56))
- **`calculateParseErrorPenalty` is language-independent scoring policy.** It is anchored to `coredomain.GradeAThreshold`/`GradeDThreshold` and now exists verbatim in both tools; it belongs in `core/domain` next to `LinearPenalty` and `DuplicationPenalty` ([polyscan#57](https://github.com/ludo-technologies/polyscan/issues/57))
